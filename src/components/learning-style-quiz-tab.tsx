...
export function LearningStyleQuizTab() {
  ...
  /**
   * Efecto para cargar la estrategia desde Firestore solo en el cliente.
  ...
  useEffect(() => {
    async function fetchStrategy() {
        ...
        const strategy = await getLearningStrategy(user.uid);
        if (strategy) {
   ...
   */
  const handleQuizFinish = async (strategy: LearningStrategy) => {
    if (!user) return;
