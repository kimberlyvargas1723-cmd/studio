...
import { getFeedbackHistory } from '@/lib/services';
...
export default function ProgressPage() {
    const { user } = useUser();
    const { data: userData, loading: isLoadingPerformance } = useDoc(user ? `users/${user.uid}` : null);
    ...
    const performance: PerformanceData[] = userData?.performanceData || [];

    /**
     * Al montar el componente, este efecto carga todos los datos necesarios:
     * 1. Obtiene el feedback pasado de la IA de Firestore.
     * 2. Si hay datos de rendimiento, llama a una Acción de Servidor para generar un resumen
     *    de progreso fresco impulsado por IA.
     */
    useEffect(() => {
        // Carga el historial de feedback desde Firestore.
        if (user) {
            setIsLoadingFeedback(true);
            getFeedbackHistory(user.uid)
                .then(history => {
        ...
        // El hook `useDoc` maneja la carga de `performance`. 
        // Una vez que deja de cargar y tenemos datos, podemos generar el resumen.
        if (!isLoadingPerformance && performance.length > 0) {
            const hasPerformanceData = performance.some(p => p.correct > 0 || p.incorrect > 0);
            if (hasPerformanceData) {
                setIsLoadingSummary(true);
                generateProgressSummaryAction({ performanceData: performance })
                    .then(result => {
        ...
        } else if (!isLoadingPerformance) {
            setIsLoadingSummary(false);
        }
    }, [performance, isLoadingPerformance, user]);
