...
export default function SchedulePage() {
  ...
  }, [user]); // Depende de `user` para volver a ejecutar si el usuario cambia
  ...
  const handleGenerateStudyPlan = async (date: Date) => {
    if (!user) {
        setError('Debes iniciar sesión para generar un plan de estudio.');
        return;
    }
    setLoading(true);
    setError(null);
    ...
    try {
      const performanceData = await getPerformanceData(user.uid);
