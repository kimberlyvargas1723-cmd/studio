...
import { analyzePerformanceAndAdaptAction } from '@/app/actions';
...
export const useQuiz = ({ quiz, onQuizFeedback, learningStyle }: UseQuizProps) => {
  ...
  const handleAnswerSubmit = async () => {
    if (!selectedAnswer || !user) return; // No hacer nada si no hay respuesta o usuario
    ...
    // Actualiza los datos de rendimiento en Firestore.
    await updatePerformanceData(user.uid, currentQuestion.topic, correct);
    ...
    if (!quiz.isPsychometric) {
      try {
        const result = await analyzePerformanceAndAdaptAction({
        ...
        if (questionIndexAtSubmission === currentQuestionIndex) {
            ...
            await saveFeedback(user.uid, newFeedback); // Guarda en Firestore
        }
      } catch (error) {
